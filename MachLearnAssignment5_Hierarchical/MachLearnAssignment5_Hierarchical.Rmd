---
title: "Assignment4_Hierarchical_Clustering"
author: "Abhishek_Sau"
date: "19/04/2021"
output:
  html_document:
...
pltree(hc4, cex = 0.6, hang = -1, main = "Dendrogram of ward", labels = cereals_df2$`cereals_df1$name`)
...
```
As per the "Elbow plot", I will choose two clusters.
...
cluster_B <- data.frame(data=seq(1,nrow(cereal_part2),1),part2_cluster = rep(0,nrow(cereal_part2)))

for(x in 1:nrow(cereal_part2))
{ cluster_B$part2_cluster <- which.min(as.matrix(get_dist(as.data.frame(rbind(centroid[,-c(1:7,25)],cereal_part2[x,c(-1:-8, -26)]))))[3,-3])}

cluster_B <- cluster_B %>% mutate(clusters = cereal_part2$cluster)

mean(cluster_B$part2_cluster==cluster_B$clusters)
